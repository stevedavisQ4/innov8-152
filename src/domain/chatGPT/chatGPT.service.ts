...
export class ChatGTPExecutor {
  ...
  public async createTestCase(
    ...
    try {
      ...
      const response = await this.openAIService.query(
        preAmble + promptExpression + postAmble
      );
      ...
      console.error(
        `[ChatGTPExecutor] It was not possible to create completetion due: ${err.message}`
      );
      return null;
    }
  }
  ...
  public async createTestCaseFromScripts(scriptDir: string): Promise<void> {
    ...
    const existingCases = Object.entries(prompts).map(([key, value]) =>
      this.createTestCase(key, value)
    );
